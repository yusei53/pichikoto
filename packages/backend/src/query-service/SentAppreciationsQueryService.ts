...
export class SentAppreciationsQueryService {
  async getByDiscordUserId(
    discordUserId: string
  ): Promise<
    Result<AllAppreciationsResponse, SentAppreciationsQueryServiceError>
  > {
    try {
      // 指定されたユーザーが送信した感謝投稿を作成日時の降順で取得
      ...
      return err(
        new SentAppreciationsQueryServiceError(
          "Failed to fetch sent appreciations",
...
export class SentAppreciationsQueryServiceError extends Error {
