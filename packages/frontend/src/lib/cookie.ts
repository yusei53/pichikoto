export const cookieUtils = {
  set: (name: string, value: string, options: { maxAge?: number; secure?: boolean; httpOnly?: boolean; sameSite?: 'strict' | 'lax' | 'none' } = {}) => {
  ...
  auth: {
    setTokens: (accessToken: string, refreshToken: string) => {
      // アクセストークン: 30日間（バックエンドのJWTサービスと同じ有効期限）
      cookieUtils.set('accessToken', accessToken, { 
        maxAge: 60 * 60 * 24 * 30,
        secure: true,
        sameSite: 'strict'
      });
      // リフレッシュトークン: 1年間（バックエンドのJWTサービスと同じ有効期限）
